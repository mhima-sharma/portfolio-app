...
@Component({
  selector: 'app-corporate-professional-theme',
  standalone: true,
  imports: [CommonModule, RouterLink],
  template: `
    <div class="corporate-professional min-h-screen flex flex-col bg-slate-100 text-slate-900">
      <header class="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-md">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <p class="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Corporate Professional</p>
          ...
          <nav class="hidden items-center gap-5 text-sm font-semibold text-slate-700 md:flex">
            @for (item of navItems; track item.page) {
              <a [routerLink]="['/', profileSlug(), item.page]" [class.text-sky-600]="page() === item.page" class="transition hover:text-sky-600">{{ item.label }}</a>
            }
          </nav>
        </div>
      </header>
        ...
        @if (page() === 'home') {
          <section class="rounded-[2rem] bg-white p-8 shadow-sm sm:p-10">
            <div class="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
              <div>
                <p class="text-sm uppercase tracking-[0.28em] text-sky-600">Home</p>
                <h2 class="mt-4 text-4xl font-bold text-slate-900 sm:text-5xl">Professional visuals for business leaders.</h2>
                <p class="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{{ data().about.description }}</p>
                <div class="mt-8 flex flex-wrap gap-4">
                  <a [routerLink]="['/', profileSlug(), 'about']" class="rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">About</a>
                  <a [routerLink]="['/', profileSlug(), 'projects']" class="rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-800 hover:text-white">Projects</a>
                </div>
              </div>
              <div class="rounded-[1.5rem] bg-slate-50 p-6">
                <p class="text-xs uppercase tracking-[0.28em] text-slate-500">Value proposition</p>
          ...
          <section class="rounded-[2rem] bg-white p-8 shadow-sm sm:p-10">
            <p class="text-sm uppercase tracking-[0.28em] text-sky-600">About</p>
            <h2 class="mt-4 text-3xl font-bold text-slate-900">Structured profile for executive communication.</h2>
              ...
              <div class="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-6">
                <div class="space-y-4">
                  <div>
                    ...
                    <p class="mt-2 text-sm text-slate-900">{{ data().contact.email }}</p>
                  </div>
                  <div>
                    <p class="text-xs uppercase tracking-[0.28em] text-slate-500">Location</p>
                    <p class="mt-2 text-sm text-slate-900">{{ data().contact.location }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section class="rounded-[2rem] bg-white p-8 shadow-sm sm:p-10">
            <p class="text-sm uppercase tracking-[0.28em] text-sky-600">Projects</p>
            <h2 class="mt-4 text-3xl font-bold text-slate-900">Business-focused portfolio.</h2>
            ...
            @if (galleryImages().length) {
              <div class="mt-10 rounded-[1.5rem] border border-slate-200 bg-white p-6">
                <p class="text-xs uppercase tracking-[0.35em] text-sky-600">Gallery</p>
                <h3 class="mt-3 text-2xl font-semibold text-slate-900">Premium image gallery</h3>
                <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  @for (image of galleryImages(); track image.id) {
                    <div class="overflow-hidden rounded-[1.5rem] border border-slate-100 bg-slate-50 shadow-sm">
          ...
          <section class="rounded-[2rem] bg-white p-8 shadow-sm sm:p-10">
            <p class="text-sm uppercase tracking-[0.28em] text-sky-600">Skills</p>
            <h2 class="mt-4 text-3xl font-bold text-slate-900">Capabilities at a glance.</h2>
          ...
          <section class="rounded-[2rem] bg-white p-8 shadow-sm sm:p-10">
            <p class="text-sm uppercase tracking-[0.28em] text-sky-600">Contact</p>
            <h2 class="mt-4 text-3xl font-bold text-slate-900">Client-ready communications.</h2>
            <div class="mt-8 grid gap-8 lg:grid-cols-[0.9fr_0.8fr]">
              <div class="space-y-6">
                <p class="text-lg leading-8 text-slate-600">Available for corporate strategy, consulting, and product leadership.</p>
                <div class="space-y-4">
                  @for (link of contactLinks(); track link.key) {
                    <div class="rounded-[1.5rem] bg-slate-50 p-5 border border-slate-200">
                      ...
                      @if (link.href) {
                        <a [href]="link.href" target="_blank" rel="noopener noreferrer" class="mt-2 block text-sm text-slate-900 hover:text-sky-600 break-all">{{ link.value }}</a>
                      } @else {
                        <p class="mt-2 text-sm text-slate-900 break-words">{{ link.value }}</p>
                      }
                    </div>
                  }
                </div>
              ...
              <div class="rounded-[1.5rem] bg-slate-50 p-6">
                <form>
                  ...
                  <input class="mt-3 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" type="text" placeholder="Your name" />
                  <label class="mt-5 block text-sm font-semibold text-slate-700">Email</label>
                  <input class="mt-3 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" type="email" placeholder="Your email" />
                  <label class="mt-5 block text-sm font-semibold text-slate-700">Message</label>
                  <textarea class="mt-3 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" rows="5" placeholder="Project brief"></textarea>
                  <button type="button" class="mt-6 inline-flex w-full items-center justify-center rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Send message</button>
                </form>
              </div>
            </div>
          </section>
      ...
      <footer class="border-t border-slate-200 bg-slate-100/90 py-6">
        <div class="mx-auto flex max-w-7xl flex-col gap-4 px-4 text-sm text-slate-500 sm:px-6 lg:px-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div class="flex flex-wrap gap-4">
            <a [routerLink]="['/', profileSlug(), 'home']" class="transition hover:text-sky-600">Home</a>
            <a [routerLink]="['/', profileSlug(), 'about']" class="transition hover:text-sky-600">About</a>
            <a [routerLink]="['/', profileSlug(), 'contact']" class="transition hover:text-sky-600">Contact</a>
          </div>
        </div>
      </footer>
    </div>
...
export class ThemeCorporateProfessionalComponent {
